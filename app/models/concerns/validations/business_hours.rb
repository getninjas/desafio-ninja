class Validations::BusinessHours < ActiveModel::Validator
  def validate(record)
    @record = record
  ...
  def validate_business_hours
    if before_nine_am? || after_six_pm? || in_weekend?
      record.errors.add(:start_at, "meeting must be in business hours") 
    end
  end
  def in_weekend?
